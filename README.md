...
**UTM 만으로는 못 잰다.** UTM 은 주소 뒤에 붙는 글자일 뿐이고, 그걸 기록해 주는 건
목적지 서버다. 카카오는 남의 서버라 우리한테 아무것도 안 알려준다.
그래서 **내가 가진 문을 한 번 거쳐서** 나가야 한다.

```
인스타 바이오 → /go/openchat?from=bio → open.kakao.com/o/xxxx
                 ↑ 여기서 센다            ↑ UTM 안 붙인다
```

* 주소는 **환경변수 `OPENCHAT_URL`** 에만 둔다. 방을 새로 파도 배포 없이 바꾼다.
  안 넣으면 `/go/openchat` 이 404 다 — 빈 링크가 바이오에 걸리는 것보다 낫다
* **카카오 주소에는 UTM 을 안 붙인다.** 카카오가 읽지도 알려주지도 않아서 주소만 지저분해진다.
  세는 건 우리 쪽에서 끝난다
* `?from=` 이 곧 갈래다. 바꿔 달면 어디서 들어왔는지 갈린다

```
/go/openchat?from=bio          인스타 바이오
/go/openchat?from=story        스토리
/go/openchat?from=reel-0827    그날 릴스
```

**보는 곳은 `/insight`** — `나간 클릭` 에 `openchat (bio)` 처럼 뜬다. 사람 수·지역·기기도
다른 클릭과 똑같이 붙는다. 하루치 리포트에도 같이 들어간다.

### 미리보기 크롤러는 안 센다

인스타·카카오는 **링크가 공유될 때마다** 미리보기를 만들려고 주소를 한 번 연다.
안 거르면 아무도 안 눌러도 숫자가 오른다. → `lib/db.ts` 의 `isBot()`

**앱 안 브라우저는 거르면 안 된다.** 인스타·카톡 앱에서 열면 UA 에 `Instagram`,
`KAKAOTALK` 이 그대로 들어 있는데 그건 진짜 사람이다. 긁는 쪽은 `facebookexternalhit`,
`kakaotalk-scrap` 처럼 이름이 따로 있어서 그것만 집는다.
**이걸 뭉뚱그리면 유입의 대부분이 통째로 날아간다.**

봇 필터는 `/go` 만이 아니라 방문 집계에도 걸린다. 넣은 뒤로 숫자가 살짝 줄어 보이면
그건 원래 안 세야 했던 것이 빠진 것이다.
