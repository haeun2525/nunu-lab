...
* 겹침은 **띄우거나 돌려 놓은 것**(position≠static · transform · float)이 낀 쌍만 잡는다.
  흐름대로 쌓인 큰 글씨 네 줄은 줄상자가 서로 물려도 글자는 안 부딪히므로 뺀다.
* 겹침을 잴 때 배경·테두리가 있는 것은 상자 전체를, 없는 것은 **글자가 그려진 자리**만 본다.
  안 그러면 한 줄을 가로로 다 차지하는 링크가 오른쪽 빈 자리까지 겹쳤다고 잡힌다.
